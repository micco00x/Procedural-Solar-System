...
the distance, allowing high performance even with a high number of details.
...
a hypothetical sphere. Hence, for a point $(x, y, z)$ on the sphere corresponds
a height $h$.
...
the lacunarity and the scale are constant value (choosen before the execution
of the function). The scale value directly changes the frequency of the function.
...
renderded. A high number of chunks increases the number of details, but
...
number of vertices of each chunk. Nevertheless, this number shouldn't be
...
are not available for the vertices at the border of the mesh.
...
more times'' with different resolutions. Then, at rendering time, each LOD
...
performances while being far away from a planet and approaching it.
In particular, this allows the GPU to handle a slower number of vertices
...
distance the lights affects the objects, instead of being used to computing the attenuation
value.
...
Earth. Moreover all these objects are rotating on themselves (Figure
\ref{fig:solar-system}).

IMAGE WITH STRUCTURE OF THE SCENE ?
...
planet and is parent of all the LOD objects of the planet. Thus, instead of
