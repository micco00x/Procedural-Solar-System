...
Moon, orbiting the Earth) and a cloud of asteroids. The sun also has a particle system that simulates its surface activity. The meshes are generated using Perlin noise, each
...
position is recomputed is executed coverserly to the previous case. Since the camera must be inside the box, the distance between its position and the world origin must be lower than the radius of the space sphere box.
...
Basically these labels, placed near the associated object, are simple textured planes that keeps their rotation. To achieve this result, a label is placed on the line that passes through the camera and the corresponding planet, at a fixed distance from the latter. The label is then rotated towards the camera
keeping the rotation on the z-axis fixed.

Since the labels are objects of the scene, they are subject a the projective
transformation. This step is exploited to make further labels smaller than
the closer ones. Labels are thus scaled with respect to their distance from the camera (i.e. closer labels have a smaller scaling factor than the further ones).
...
The previously introduced procedural planet system deals with the generation of mountains and valleys for celestial bodies; at the top of that textures are applied in order to give to the planets or stars a more suitable appereance. However for a planet like the earth these steps are not enough.\\
